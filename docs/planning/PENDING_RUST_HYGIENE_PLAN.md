...
> scope is known and small; §4 asks one question before it is executable.
...
- **Toolchain pinning is a prerequisite, and it has already landed.** rustfmt
  and clippy output vary by toolchain version, so gating on them without a pin
  makes CI fail whenever the runner's stable moves. `rust/rust-toolchain.toml`
  pins `1.97.0` with `rustfmt` and `clippy` declared — it arrived with the
  release strategy work, which needed it for reproducible WASM output
  (`done/DONE_RELEASE_STRATEGY_PLAN.md` §3). Nothing further is needed here.
...
1. `cargo fmt` as a single mechanical commit, nothing else in it, so review is
   "formatting only" and the churn is never mixed with logic. Expect ~880 lines.
2. Fix the six clippy findings as a second commit, one concern per hunk.
3. Re-run `make test` — both suites, so the vector corpus proves the primitives
...
4. Add `rust-format-check` and `rust-lint` to `verify`; remove the note the
   Makefile currently prints.
5. Add both to the CI job.

The toolchain pin is already in place, so steps 1 and 2 are reproducible and
step 5 will not break when the runner's stable moves.
