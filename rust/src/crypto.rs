...
use crate::utils::{generate_random_bytes, CryptoError};
...
pub fn encrypt_for_public_key(
    ...
    let nonce: [u8; 12] = nonce.try_into().map_err(|_| CryptoError::RandomFailed)?;
...
pub fn is_usable_x25519_public_key(key: &[u8; 32]) -> bool {
    ...
    probe
        .diffie_hellman(&PublicKey::from(*key))
        .was_contributory()
}
...
mod tests {
    ...
    fn test_large_data_encryption_performance() {
        ...
        for size in test_sizes {
            ...
            assert_eq!(
                encrypted.len(),
                expected_size,
                "Encrypted size mismatch for {} bytes",
                size
            );
            ...
            if size >= 100_000 {
                // Only check performance for larger data
                assert!(
                    encrypt_duration.as_millis() < 1000,
                    "Encryption too slow for {} bytes: {}ms",
                    size,
                    encrypt_duration.as_millis()
                );
                assert!(
                    decrypt_duration.as_millis() < 1000,
                    "Decryption too slow for {} bytes: {}ms",
                    size,
                    decrypt_duration.as_millis()
                );
            }
    ...
    fn test_encryption_shared_vectors() {
        ...
        let path = concat!(env!("CARGO_MANIFEST_DIR"), "/../vectors/encryption.json");
